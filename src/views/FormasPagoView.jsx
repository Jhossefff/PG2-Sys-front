...
export default function FormasPagoView() {
  ...
  const handleSave = async (payload) => {
    ...
    try {
      if (selected) {
        await updateFormaPago(selected.idforma_pago, payload);
        setFlash("Forma de pago actualizada.");
      } else {
        await createFormaPago(payload);
        setFlash("Forma de pago creada.");
      }
      await cargar();
      setShow(false);
  ...
  const confirmDelete = async () => {
    setDeleting(true);
    try {
      await deleteFormaPago(toDelete.idforma_pago);
        ...
        <Card.Body>
          <Row className="mb-3 align-items-center">
            ...
            <Col className="text-end">
              <Button variant="primary" onClick={openNew}>+ Nueva forma</Button>
            </Col>
          </Row>
